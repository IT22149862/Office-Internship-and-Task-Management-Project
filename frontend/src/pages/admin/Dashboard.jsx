...
const PIE_COLORS = ['#7A85A6', '#3554D1', '#EE9A3A', '#E5484D', '#1F9D6B'];
...
export default function AdminDashboard() {
  ...
  return (
    <Layout title="Dashboard" subtitle="A live snapshot of every intern, project, and task">
      {error && <div className="banner banner-error">{error}</div>}

      {data && (
        <>
          <div className="stat-grid">
            <StatCard label="Active Interns" value={data.activeInterns} max={Math.max(data.activeInterns, 10)} color="var(--indigo)" />
            <StatCard label="Active Projects" value={data.activeProjects} max={Math.max(data.activeProjects, 10)} color="var(--amber)" />
            <StatCard label="Pending Tasks" value={data.pendingTasks} max={Math.max(totalTasks, 1)} color="var(--slate)" />
            <StatCard label="Completed Tasks" value={data.completedTasks} max={Math.max(totalTasks, 1)} color="var(--emerald)" />
            <StatCard label="Overdue Tasks" value={data.overdueTasks} max={Math.max(totalTasks, 1)} color="var(--ruby)" />
          </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {data.recentActivity.map((a, i) => (
                      <div key={i} className="flex-row" style={{ alignItems: 'flex-start' }}>
                        <div style={{ width: 6, height: 6, borderRadius: 3, background: 'var(--indigo)', marginTop: 6, flexShrink: 0 }} />
