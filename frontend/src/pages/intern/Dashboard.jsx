...
export default function InternDashboard() {
  ...
  return (
    <Layout eyebrow="Your Workspace" title={`Welcome, ${user?.fullName?.split(' ')[0] || 'there'}`} subtitle="Here's where things stand today">
      {error && <div className="banner banner-error">{error}</div>}

      {data && (
        <>
          <div className="stat-grid">
            <StatCard hero label="My Projects" value={data.myProjects} max={Math.max(data.myProjects, 5)} />
            <StatCard label="To Do" value={data.todoTasks} max={Math.max(totalTasks, 1)} color="var(--slate)" />
            <StatCard label="In Progress" value={data.inProgressTasks} max={Math.max(totalTasks, 1)} color="var(--gold)" />
            <StatCard label="Awaiting Review" value={data.submittedTasks} max={Math.max(totalTasks, 1)} color="var(--info)" />
            <StatCard label="Needs Revision" value={data.revisionRequiredTasks} max={Math.max(totalTasks, 1)} color="var(--danger)" />
            <StatCard label="Completed" value={data.completedTasks} max={Math.max(totalTasks, 1)} color="var(--success)" />
          </div>
